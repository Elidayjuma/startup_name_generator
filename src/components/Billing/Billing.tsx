...
const Billing: React.FC<BillingProps> = ({ subscription, usage_Data }) => {
  ...
  useEffect(() => {
    const fetchPromptUsage = async () => {
      if (usage_Data) {
        try {
          const data = await returnUserPromptUsage(usage_Data);
          setPromptsUsages(data || []);
        } catch (error) {
  ...
  return (
    <>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
        <CardDataStats title="Monthly post limit" total={subscription.monthlyUsage} rate="Top Up" levelUp>
          <svg
        ...
        <CardDataStats
          title="Posts Used"
          total={isLoading ? "Loading..." : `${promptsUsages.length}`}
          rate="Top Up"
          levelUp>
          <svg
            className="fill-primary dark:fill-white"
