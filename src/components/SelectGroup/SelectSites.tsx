...
const SelectSites: React.FC<{ onSiteSelect: (siteId: string) => void }> = ({ onSiteSelect }) => {
    const [selectedOption, setSelectedOption] = useState<string>("");
    const [isOptionSelected, setIsOptionSelected] = useState<boolean>(false);
    const [sites, setSites] = useState<any[]>([]); // Type as needed
    ...
    const handleSelect = (event: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        setSelectedOption(siteId);
        setIsOptionSelected(true);
        onSiteSelect(siteId); // Notify parent of selected siteId
    };

    useEffect(() => {
        const fetchUserSites = async () => {
            // Fetch the data and set state
            const userSites = await returnUserSites();
            setSites(userSites);
            setLoading(false); // Set loading to false once data is fetched
        };

        fetchUserSites().catch((error) => {
            ...
            <div className="relative z-20 bg-transparent dark:bg-form-input">
                ...
                >
                    ...
                    {sites.map((site, index) => {
                        return (
                            <option key={index} value={site.id} className="text-body dark:text-bodydark">
                                {site.site_name}
                            </option>
                        )
                    })
